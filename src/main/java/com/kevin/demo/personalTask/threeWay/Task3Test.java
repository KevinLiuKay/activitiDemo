...
public class Task3Test {
	...
	public void deploymentProcessDefinition_inputStream() throws Exception{
		...
		Deployment deployment = processEngine.getRepositoryService() //与流程定义和部署对象相关的Service
												...
												.deploy();//完成部署
		System.out.println("部署ID:" + deployment.getId());	//部署ID:5701
		System.out.println("部署名称:" + deployment.getName());//部署名称:任务3
	}
	...
	public void startProcess () throws Exception{
		...
		String processDefinitionKey = "task3";
		ProcessInstance pi = processEngine.getRuntimeService() //与正在执行的流程实例和执行对象相关的Service
							.startProcessInstanceByKey(processDefinitionKey);//使用流程定义的key启动流程实例，key对应helloworld.bpmn文件中的id的属性值
		System.out.println("流程实例ID:" + pi.getId());//流程实例ID:5801
		System.out.println("流程定义ID:"+ pi.getProcessDefinitionId());	//流程定义ID: task3:1:5704
	}
