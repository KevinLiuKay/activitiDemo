package com.kevin.demo.groupTask.oneWay;
...
public class GroupTask1Test {
	...
	public void deploymentProcessDefinition_inputStream() throws Exception{
		...
		//获取上传文件输入流程 
		InputStream inputStreamBpmn = this.getClass().getResourceAsStream("groupTask1.bpmn");
		InputStream inputStreamPng = this.getClass().getResourceAsStream("groupTask1.png");
		//获取仓库服务，从类路径下完成部署
		Deployment deployment = processEngine.getRepositoryService() //与流程定义和部署对象相关的Service
												.createDeployment()	//创建一个部署对象
												.name("组任务") //添加部署名称
												.addInputStream("groupTask1.bpmn", inputStreamBpmn)
												.addInputStream("groupTask1.png", inputStreamPng)
												.deploy();//完成部署
		System.out.println("部署ID:" + deployment.getId());	//部署ID:6001
	...
	public void startProcess () throws Exception{
		...
		String processDefinitionKey = "groupTask1";
		ProcessInstance pi = processEngine.getRuntimeService() //与正在执行的流程实例和执行对象相关的Service
							.startProcessInstanceByKey(processDefinitionKey);//使用流程定义的key启动流程实例，key对应helloworld.bpmn文件中的id的属性值
		System.out.println("流程实例ID:" + pi.getId());//流程实例ID:6101
		System.out.println("流程定义ID:"+ pi.getProcessDefinitionId());	//流程定义ID: groupTask1:1:6004
	}
